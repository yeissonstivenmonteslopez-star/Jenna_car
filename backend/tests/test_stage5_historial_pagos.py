...
def test_user_can_download_own_receipt_as_pdf():
    state = make_payment_state('pdf1')
    try:
        client = backend_app.app.test_client()
        response = client.get(
            f'/api/recibos/{state["receipt_id"]}/pdf',
        ...
        assert 'recibo-ORDEN-' in response.headers['Content-Disposition']
    finally:
        cleanup_payment_state(state)
