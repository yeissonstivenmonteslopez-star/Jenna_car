...
import { getAdminUsuarios, bulkDeleteUsuarios, updateUsuarioRol } from '@/features/usuarios/services/usuariosService'
...
export default function AdminUsuarios() {
  ...
  const handleEliminarSeleccionados = async () => {
    ...
    let data: any
    try { data = await bulkDeleteUsuarios(selectedIds, token) } catch (err: any) { return setMessage(err.message || 'Error al eliminar usuarios') }
  ...
  const handleCambiarRol = async (usuarioId: number) => {
    if (!token) return
    try {
      await updateUsuarioRol(usuarioId, newRol, token)
      setMessage(`Rol cambiado a ${newRol}`)
      setNewRol('usuario')
      setEditing(null)
      const payload = await getAdminUsuarios(token)
      setUsers(payload.data || [])
    } catch (err: any) { return setMessage(err.message || 'Error al cambiar rol') }
  }
                  ...
                  filteredUsers.map((user) => (
                    <tr key={user.id} className="border-b border-white/10 text-sm text-white/80">
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          {user.foto_perfil ? (
                            <img src={buildAssetUrl(user.foto_perfil)} alt={`${user.nombre} ${user.apellido}`} className="h-10 w-10 rounded-full object-cover" />
                          ) : (
