...
import { getReciboPdfUrl } from '@/features/recibos/services/recibosService'
import { getServices } from '@/features/servicios/services/serviciosService'
import { getOrdenes, createOrden, updateOrden, deleteOrden } from '@/features/recibos/services/pagosService'
...
export default function AdminOrdersPage() {
  ...
  async function loadData(authToken: string) {
    const [orderData, clientData, vehicleData, serviceData] = await Promise.all([
      getOrdenes(authToken),
      adminSearch({ q: '', type: 'cliente' }, authToken),
      adminSearch({ q: '', type: 'vehiculo' }, authToken),
      getServices(),
    ])
  ...
  useEffect(() => {
    ...
    loadData(token).catch((error) => setMessage(error instanceof Error ? error.message : 'No fue posible cargar los datos.')).finally(() => setLoading(false))
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (selectedId) await updateOrden(selectedId, payload, token)
      else await createOrden(payload, token)
    } catch (err: any) { setMessage(err.message || 'No fue posible guardar la orden.'); return }
  ...
  async function remove(orderId: number) {
    try { await deleteOrden(orderId, token) } catch (err: any) { setMessage(err.message || 'No fue posible eliminar la orden.'); return }
