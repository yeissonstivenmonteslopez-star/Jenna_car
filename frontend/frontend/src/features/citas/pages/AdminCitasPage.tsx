...
import { getAdminCitas, updateCitaEstado } from '@/features/citas/services/citasService'
...
export default function AdminCitas() {
  ...
  const fetchCitas = async (authToken: string) => {
    const data = await getAdminCitas(authToken)
    setCitas(data.data || [])
  }
  ...
  const cambiarEstadoRapido = async (citaId: number, estado: 'confirmada' | 'cancelada') => {
    ...
    setMessage('')
    try {
      const data = await updateCitaEstado(citaId, estado, token).catch((err: any) => ({ error: err.message }))
      if ((data as any).error) { setMessage((data as any).error); return }
  ...
  const handleCambiarEstado = async (citaId: number) => {
    if (!token) return
    try {
      await updateCitaEstado(citaId, newEstado, token)
      setMessage(`Estado cambiado a ${newEstado}`)
    } catch (err: any) {
      setMessage(err.message || 'Error al cambiar estado'); return
    }
    setEditingId(null)
    await fetchCitas(token)
  }
