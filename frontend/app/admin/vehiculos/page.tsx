...
export default function AdminVehiculosPage() {
  ...
  async function loadClients(authToken: string) {
    const response = await fetch(`${apiUrl}/api/admin/search?type=cliente&q=`, {
      headers: { Authorization: `Bearer ${authToken}` },
    })
    if (!response.ok) throw new Error('No fue posible cargar los clientes.')
    const data = await response.json()
    setClients((data.data?.clientes || []).map((item: any) => ({
      id: item.id,
      nombre: item.nombre,
  ...
  async function loadVehicles(authToken: string) {
    const response = await fetch(`${apiUrl}/api/admin/vehiculos?q=${encodeURIComponent(search)}`, {
      headers: { Authorization: `Bearer ${authToken}` },
    })
    if (!response.ok) {
      const data = await response.json().catch(() => ({}))
      throw new Error(data.error || 'No fue posible cargar los vehículos.')
    }
    const data = await response.json()
  ...
  useEffect(() => {
    ...
    Promise.all([loadClients(storedToken), loadVehicles(storedToken)])
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`${apiUrl}/api/admin/vehiculos${selectedId ? `/${selectedId}` : ''}`, {
    ...
    const data = await response.json().catch(() => ({}))
    if (!response.ok) {
      setMessage(data.error || 'No fue posible guardar el vehículo.')
      return
    }
  ...
  async function removeVehicle(vehicleId: number) {
    ...
    const response = await fetch(`${apiUrl}/api/admin/vehiculos/${vehicleId}`, {
      method: 'DELETE',
    ...
    const data = await response.json().catch(() => ({}))
    if (!response.ok) {
      setMessage(data.error || 'No fue posible eliminar el vehículo.')
      return
    }

    setMessage(data.message || 'Vehículo actualizado.')
    resetForm()
    await loadVehicles(token)
  }
  ...
  async function toggleStatus(vehicleId: number, nextState: string) {
    ...
    const data = await response.json().catch(() => ({}))
    if (!response.ok) {
      setMessage(data.error || 'No fue posible cambiar el estado del vehículo.')
      return
    }
    setMessage(data.message || 'Estado del vehículo actualizado.')
    await loadVehicles(token)
  }
