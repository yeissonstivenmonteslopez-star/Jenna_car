...
export default function ProfilePage() {
  ...
  async function upload(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/usuarios/perfil/foto`, {
        method: 'POST',
      ...
      if (!response.ok) {
        setMessage(data.error || 'No fue posible actualizar la foto.')
        event.target.value = ''
        return
      }
      setUser(data.user)
  ...
  async function updateProfile(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const data = await response.json()
      if (!response.ok) {
        setMessage(data.error || 'No fue posible actualizar el perfil.')
        return
      }
      setUser(data.user)
      localStorage.setItem('jenna_car_user', JSON.stringify(data.user))
